			...
			unsigned int mss)
{
	...
	i = tx_ring->next_to_use;
        if (skb->len == 81) {
        	printk(KERN_DEBUG "DANX: %s:%s:%d, Ring index = %d, Len = %d \n", 
               		__FILE__ , __func__ ,__LINE__, i, skb->len);
	}
	...
	tx_ring->buffer_info[first].next_to_watch = i;
        ...
        if (skb->len == 81) {
        	printk(KERN_DEBUG "DANX: %s:%s:%d, Byte count = %u, Len = %d \n", 
               		__FILE__ , __func__ ,__LINE__, bytecount, skb->len);
	}
				    ...
				    struct net_device *netdev)
{
	 ...
	 */
        ...
        printk(KERN_DEBUG "DANX: %s %s:%s:%d, Details : Len = %d \n" 
               ,netdev->name, __FILE__ , __func__ ,__LINE__,
               skb->len);
        if (strcmp(netdev->name, "eth4") == 0) {
                unsigned long long now = ktime_to_ns(ktime_get_real());
        	printk(KERN_DEBUG "DANX: %llu %s %s:%s:%d, Details : Len = %d \n" 
        	       ,now
               	       ,netdev->name, __FILE__ , __func__ ,__LINE__,
                       skb->len);
	...
	if (count) {
		...
		skb_tx_timestamp(skb);
                ...
                if (strcmp(netdev->name, "eth4") == 0) {
        		printk(KERN_DEBUG "DANX: TIMETSAMP %s %s:%s:%d, Details : Len = %d \n" 
               			,netdev->name, __FILE__ , __func__ ,__LINE__,
               			skb->len);
		}
...
static irqreturn_t e1000_intr(int irq, void *data)
{
	...
	u32 icr = er32(ICR);
        ...
        if (strcmp(netdev->name, "eth4") == 0) {
        	/*printk(KERN_DEBUG "DANX: start %s %s:%s:%d\n" 
                       ,netdev->name, __FILE__ , __func__ ,__LINE__);*/
	}
	if (unlikely((!icr)))
...
static void e1000_netpoll(struct net_device *netdev)
{
	struct e1000_adapter *adapter = netdev_priv(netdev);
        ...
        if (strcmp(netdev->name, "eth4") == 0) {
        	printk(KERN_DEBUG "DANX: %llu %s %s:%s:%d\n" 
        	       ,(unsigned long long)ktime_to_ns(ktime_get_real())
               	       ,netdev->name, __FILE__ , __func__ ,__LINE__);
	}
